...
def add_user(uname, uid):
    list = [uname,uid]
    with codecs.open('gamma/userlist.csv','a','utf-8') as csvfile:
...
def del_user(uname,uid):
    ...
    with codecs.open('gamma/userlist.csv','r','utf-8') as csvfile,tempfile:
        ...
        for row in reader:
            if row == list:
                ...
                writer.writerow(row)
    shutil.move(tempfile.name, 'gamma/userlist.csv')

def welcome_msg():
    return 'Welcome to our chatroom! We are the gamma and we always stay at the top.'

def list_of_files(conn, uid):
    print('Listing files to {}...'.format(uid))
    listing = os.listdir(os.getcwd()+'/gamma/files')
    conn.send(struct.pack("i", len(listing)))
    total_directory_size = 0
    for i in listing:
        conn.send(str.encode(i+':'+str(os.path.getsize(os.getcwd()+'/gamma/files/'+i))))
        total_directory_size += os.path.getsize(os.getcwd()+'/gamma/files/'+i)
        conn.recv(1024)
...
def upload_file(conn, address, uid):
    ...
    output_file = open('gamma/files/'+file_name, "wb")
    bytes_recieved = 0
    ...
    with codecs.open('gamma/log.csv','a','utf-8') as csvfile:
...
def download_file(conn, address, uid):
    ...
    print(file_name)
    if os.path.isfile(os.getcwd()+'/gamma/files/'+file_name):
        conn.send(struct.pack("i", os.path.getsize(os.getcwd()+'/gamma/files/'+file_name)))
    else:
    ...
    content = open('gamma/files/'+file_name, "rb")
    l = content.read(1024)
    while l:
        conn.send(l)
        l = content.read(1024)
...
def del_file(conn, address, uid):
    ...
    if confirm_delete == "Y":
        try:
            os.remove(os.getcwd()+'/gamma/files/'+file_name)
...
def show_logs(conn, uid):
    ...
    msg = []
    with codecs.open('gamma/log.csv','rb','utf-8') as csvfile:
...
def msgs(conn, uid):
    csvfile = codecs.open('gamma/msglog.csv', 'rb', 'utf-8')
    linereader = csv.reader(csvfile, delimiter='\t', quotechar='\"')
    while True:
        msg = []
        for row in linereader:
        ...
        if msg != 'quit':
            ...
            with codecs.open('gamma/msglog.csv','a','utf-8') as csvfile:
                linewriter = csv.writer(csvfile, delimiter = '>', quotechar = '\"')
